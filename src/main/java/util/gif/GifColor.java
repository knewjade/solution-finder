...
public enum GifColor {
    ...
    private static final EnumMap<ColorType, GifColor> map = new EnumMap<>(ColorType.class);

    static {
        for (GifColor color : GifColor.values())
            if (color.type != null)
                map.put(color.type, color);
    }

    public static GifColor parse(ColorType type) {
    ...
    GifColor(ColorType type, Color normal, Color strong) {
        this.type = type;
        this.normal = normal;
        this.strong = strong;
    }
