        ...
        try (BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(outputFile, false), charset))) {
            collect.stream()
                    .forEach(pair -> {
                        ...
                        String format = "<li><a href='http://fumen.zui.jp/?v115@%s' target='_blank'>%s %% [%d] => http://fumen.zui.jp/?v115@%s </a></li>";
                        String element = String.format(format, url, percent, count, url);
                        try {
