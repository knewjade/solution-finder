...
ある程度つくりたいTスピンを予測して、なるべく範囲が狭くなるように指定するのをおすすめします。
...
「Tスピンの屋根や回転入れのためのミノ」をみつけるのは難しく、Tスピン自体をみつけるよりも時間がかかります。

そのため、マージンの高さ（`-m` の値）を小さくしたり、ブロックのない範囲が広くならないようにしたりしてください。


3. 屋根に利用できるミノ数を少なくする

2ではマージンの範囲を狭くしましたが、マージンに置くミノの最大数（`-mr` の値）を小さくしても実行時間は短くなります。

たとえば `-mr 1` と指定すると、最低限のTスピンの形からさらに1ミノだけ置くことになります。
